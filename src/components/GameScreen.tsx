...
export const GameScreen: React.FC<GameScreenProps> = ({ equations, bpm, onGameOver }) => {
  ...
  const strokeColor = flash === 'error' ? '#FF6B6B' : flash === 'success' ? '#69DB7C' : isLowTime ? '#FFE066' : '#1e293b';

  return (
    <div className={`h-full w-full relative flex flex-col items-center justify-center pt-[80px] pb-4 px-4 transition-colors duration-300 ${flash === 'error' ? 'bg-red-100' : flash === 'success' ? 'bg-green-100' : ''}`}>
      ...
      <div className="flex-1 w-full max-w-sm relative flex flex-col items-center justify-center py-6">
        ...
        <div 
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full aspect-square border-4 border-slate-300 rounded-full z-0" 
          style={{ 
        ...
        {/* Timer Ring */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[320px] h-[320px] md:w-[360px] md:h-[360px] z-0">
           <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90 transform">
             <circle cx="50" cy="50" r="48" fill="none" stroke="#e2e8f0" strokeWidth="2.5" />
             <circle 
                cx="50" cy="50" r="48" 
                fill="none" 
                stroke={strokeColor} 
        ...
        >
          {/* Difficulty Tag */}
          <div className="absolute -top-3 -right-3 px-3 py-1 bg-yellow-200 border-2 border-slate-800 transform rotate-6 rounded text-xs font-ui font-black uppercase text-slate-800">
            {currentEq.difficulty}
          </div>

          <h2 className="text-4xl md:text-5xl font-math font-bold text-slate-800 my-8 text-center drop-shadow-sm whitespace-nowrap overflow-hidden text-ellipsis w-full">
            {currentEq.equation}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="w-full relative px-4">
            <input
              ...
              className="w-full text-center text-4xl font-math font-bold py-3 bg-slate-50 border-b-4 border-slate-300 focus:border-slate-800 focus:bg-white focus:outline-none transition-all text-slate-900 placeholder-slate-300"
              placeholder="?"
              autoFocus
              onBlur={() => {
          ...
          <div className="h-6 mt-6 w-full text-center flex items-center justify-center">
             {isLowTime && currentEq.hint && (
                <p className="inline-flex bg-slate-800 text-yellow-300 font-ui text-[10px] font-black uppercase px-2 py-1 rounded">
                   HINT: {currentEq.hint}
                </p>
             )}
          </div>
